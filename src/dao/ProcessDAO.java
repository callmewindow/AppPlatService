...
/**
 * A data access object (DAO) providing persistence and search support for Process
...
public class ProcessDAO extends BaseDAO {
    ...
    public Process findById(String id) {
        ...
        try {
            Process instance = (Process) getSession().get("entity.databaseEntity.Process", id);
            return instance;
        } catch (RuntimeException re) {
    ...
    public List findByExample(Process instance) {
        ...
        try {
            List results = getSession().createCriteria("entity.databaseEntity.Process")
