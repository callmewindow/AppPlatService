 ...
 */
public interface IDataTransAnalysisManage {
    //创建文件夹
    @GET
    @Path("/getTreeNodes")
    ...
    @GET
    @Path("/channelComput")//计算链路分析
    String channelComput(@QueryParam("params") double [] params);
    @GET
    @Path("/coverCompute")//覆盖分析计算
    String coverCompute(@QueryParam("jsonstr") String jsonstr, @QueryParam("filepath") String filepath);
    @GET
